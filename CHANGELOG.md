...
### Added
- 设备页与逐盘/逐分区自动挂载授权；全局开关改为可持久化的运行/暂停状态
- daemon 安装、启动、安全停止、重启、卸载完整生命周期控制
- `devices.*`、`auto_mount.*` RPC 与 v2 原子配置迁移

### Fixed
- 普通 U 盘可能被弱 LBA4 特征误判并提前 `unmountDisk`
- 同一物理盘多分区 session 相互覆盖、手动卸载后索引未清理
- 主窗口关闭后应用退出或无法从菜单栏恢复
- daemon 重装覆盖用户自动挂载配置
