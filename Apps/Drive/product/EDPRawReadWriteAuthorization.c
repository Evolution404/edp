...
static int receive_fd(int socket_fd) {
    ...
    ssize_t received;
    do { received = recvmsg(socket_fd, &message, 0); } while (received < 0 && errno == EINTR);
...
/// Deliberately narrow product authorization surface for writable EDP media.
/// The caller cannot choose flags: this helper always requests exactly
/// O_RDWR | O_CLOEXEC for an entire /dev/rdiskN path.
int edp_authopen_readwrite_fd(const char *path,
    ...
    if (child == 0) {
        ...
        close(sockets[1]);
        char flags[32];
        snprintf(flags, sizeof(flags), "%d", O_RDWR | O_CLOEXEC);
    ...
    if (raw_fd < 0) {
        if (WIFEXITED(status)) {
            fprintf(stderr, "EDP_READWRITE_AUTHOPEN_EXIT_STATUS=%d\n", WEXITSTATUS(status));
        } else if (WIFSIGNALED(status)) {
            fprintf(stderr, "EDP_READWRITE_AUTHOPEN_SIGNAL=%d\n", WTERMSIG(status));
        }
