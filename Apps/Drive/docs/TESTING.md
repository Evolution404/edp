...
Ordinary smoke CI still runs the complete deterministic UI render/layout/accessibility suite on GitHub with `EDP_UI_PERF_REQUIRED=0`, so hosted-runner Instruments stalls do not sit on every development critical path. The explicit manual `storage_profile=release` run launches two independent macOS 26 performance probes on fresh hosted runners. If both probes fail before producing any complete hitch trace, a third fresh-runner probe starts conditionally. A separate release-gate job requires at least one completed probe to pass the unchanged 33 ms parser; a completed trace that exceeds 33 ms is a real performance failure and does not trigger the infrastructure-only third probe.
...
3. `regression-ui`: deterministic UI on every non-nightly run; explicit release runs additionally start fresh-runner probes A/B, with C only after two infrastructure-level trace failures, and require `regression-ui-release-gate` to accept at least one complete 33 ms pass;
