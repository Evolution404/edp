...
```text
Status: RELEASE GATES PASS / CLEAN-INSTALL VERIFIED / STANDARD-EDP THREE-PARTITION PHYSICAL ACCEPTANCE PASS / EXACT-HEAD REBOOT ACCEPTANCE PASS — missing negative physical media remain BLOCKED_BY_FIXTURE
...
Exact-head reboot acceptance: PASS — reboot completed at 2026-09-06 21:00:57; logically safe-ejected still-inserted Lexar remained suppressed after reboot, service/XPC/macFUSE FSKit and saved credentials persisted, physical removal retired the old disk6/IOKit generation, fresh reinsert restored the same five-factor identity and privileged raw access, post-reboot credential/policy plus type1/type2/type4 functional-all all PASS, and final safe eject/residue audit ended with zero mounts/transport/raw holders and zero recovery counters
...
Date: 2026-09-06
Tester: automated local + GitHub Actions + current-head standard Lexar three-partition physical acceptance + exact-head reboot acceptance
```
...
- [x] `regression-ui-release-gate` PASS after at least one of `regression-ui-perf-a` / `regression-ui-perf-b` / conditional `regression-ui-perf-c` produces a complete 33 ms pass. Probe C is valid only when A and B both failed before emitting a complete hitch count.
