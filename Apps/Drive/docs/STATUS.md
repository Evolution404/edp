...
Current fixed-head CI: `ddf510b754f6e67d5c74f0fb0cbdec4fafd9f93e` / GitHub Actions run `33727607440` — **PASS**, but superseded by the exact-registry retirement hardening below.
Current physical release candidate: **PENDING REBUILD / FOCUSED RETEST**. The certificate-backed `51a6c9c` package (SHA-256 `bf4435769052ff4a8798a34d50ce406415cc4f69eb0ed6f8965cd340ac9059b7`) passed factory-clean/install/FDA/replug/service/reboot/partition persistence, but final physical acceptance found a release-blocking safe-eject bug: restarting the foreground App while the logically ejected Lexar remained physically inserted reacquired raw access. `ddf510b` persisted a logical-eject tombstone and passed all five core CI jobs, but follow-up code audit found its retirement condition still trusted discovery absence / generation change too broadly; its rebuilt package (SHA-256 `2bcfff76ee1f17f60e5ce7288b33eaf475f695819afa7145d845118596f4e4a7`) is therefore invalidated before physical release acceptance. The current hardening makes the exact persisted `usbRegistryEntryID` the sole retirement authority; S36–S40 plus local fast/system/virtual are PASS. A new exact-head CI + signed package + focused physical safe-eject/App-restart/service-restart/removal/reinsert retest is required before release-ready status.
