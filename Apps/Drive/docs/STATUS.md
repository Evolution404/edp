...
Current validated release baseline: `51a6c9c1e75e3d2dd2695c5c082a7717a010f12d` — **invalidated candidate evidence only**
Current fixed-head CI: `54c048f39ddab9bd22e1cc01bd7d7276a75d3a0f` / GitHub Actions run `33745043903` — **PASS** (native, fast, virtual USB, UI-system and storage M01–M14). Its signed package SHA-256 is `cbb9b83bf25b60971e79136123b3700548f83e79b7610e399dae708d6ad13a8b`, but that candidate is invalidated by the service-lifecycle claim-gap blocker below.
Current physical release candidate: **PENDING CLAIM-CONTINUOUS RUNTIME-CONTROL REBUILD / FOCUSED RETEST**. `f7d7dde` first closed safe-eject App/service reacquisition with exact persisted `usbRegistryEntryID` tombstone retirement (S36–S40), but physical reinsert exposed macOS 26 `fskitd` holding `/dev/rdisk6s1` and forcing whole-disk `O_RDWR` to fail with `EDP_RAW_LEASE_OPEN_FAILED:16`. `54c048f` then added standard-EDP-only Disk Arbitration peek/claim (S41). Fresh physical replug of the installed `54c048f` package proved that prevention works: `DA_CLAIMED=true`, root `lsof` showed only `edp-drive-service` holding `/dev/rdisk6`, no `fskitd` child-partition holder existed, raw access became ready directly with `rawBusyRecoveryCount=0` and `forcedWholeUnmountCount=0`, and the five-factor Lexar identity plus saved credentials/policy remained unchanged. Foreground App restart preserved the claim. A true privileged-service stop/start then exposed the next blocker: terminating the service destroyed its Disk Arbitration session/claim, `fskitd` immediately reopened `/dev/rdisk6s1`, and the restarted service returned to EBUSY. The current uncommitted hardening therefore changes routine Stop/Start/Restart to in-process runtime pause/resume/restart: mounts/transports/raw leases quiesce, but the privileged process and DA claim remain continuous; explicit Complete Quit resumes if needed, safe-ejects all connected EDP devices, then performs the true process shutdown. S42/S43, native `-warnings-as-errors` build, fast/virtual/system and the 320000-step model are PASS locally. A new exact-head CI + signed package + physical pause/resume/restart/full-exit retest is required before release-ready status.
