...
Current fixed-head CI: `f7d7dde3f0bf5d82e5f348627b1e0836228bbdae` / GitHub Actions run `33734920686` — **PASS** (native, fast, virtual USB, UI-system and storage M01–M14), but the package is invalidated by the physical reinsert EBUSY blocker below.
Current physical release candidate: **PENDING EARLY-CLAIM REBUILD / FRESH REPLUG RETEST**. `f7d7dde` made the exact persisted `usbRegistryEntryID` the sole safe-eject tombstone retirement authority; S36–S40 passed, and its package SHA-256 `7a9d7f745eceda43b6d3707f6b7b6c02a8ffd2f9de6f12341f93b9a680f1ca04` physically passed safe eject, foreground App restart while still inserted, and full privileged-service stop/start while still inserted without raw reacquisition. After real removal/reinsert, however, Drive recognized the same five-factor Lexar identity but could not reopen the whole raw device: `EDP_RAW_LEASE_OPEN_FAILED:16`. Root-level `lsof` proved `fskitd` retained `/dev/rdisk6s1`; force whole-unmount, manual raw refresh, and service restart did not release that holder. Therefore `f7d7dde` / `7a9d...` is not release-ready. The current hardening registers a Disk Arbitration peek callback and claims only verified `standardEncrypted` EDP whole media before automatic FSKit probing; unverified, ordinary, legacyNoPassword, currentNoPassword and unrecognized media remain macOS-owned. S41 plus local fast/virtual/system are PASS. Exact-head CI, signed package rebuild/install, and a fresh physical remove/reinsert + safe-eject lifecycle retest are still required.
