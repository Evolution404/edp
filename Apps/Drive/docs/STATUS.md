...
Current release code/package HEAD: `84507259ec35f3ade234ab31f2caa256115dc124`
Current exact-head CI: GitHub Actions run `34029328625` — **PASS 5/5** core paths (native, fast+VirtualUSB, deterministic UI, storage core and storage lifecycle), with the separate UI release performance gate also PASS. Release storage proved M10 5/5 with no mount/device/process/fd leak, M12 crash/remount PASS, and M14 concurrent boot/exchange/secure teardown PASS. UI probe A produced a complete `UI_HITCH_COUNT_GT33MS=0` trace; probe B failed before trace completion at `xctrace record`; conditional fresh-runner probe C correctly skipped because A already supplied authoritative performance evidence.
Current signed package: `artifacts/EDP-Drive-0.6.0-arm64-Clean.pkg`, SHA-256 `322ce7aff9f18a680f62d6675a5e951a1e0e9bb681a8d1cc8a04984409ef8117` — strict release verifier PASS. A full EDP/macFUSE environment cleanup followed by installation of this exact package passed App/service signing, LaunchDaemon loading, privileged XPC roundtrip and version `0.6.0` snapshot with no external physical disk attached.
Current candidate status: **AUTOMATED RELEASE GATES PASS / CLEAN-INSTALL VERIFIED; exact-head physical USB and reboot acceptance pending.** Healthy transport teardown now uses bounded ordinary VFS unmount plus exact IOMedia-generation termination before adapter exit; forced unmount remains outside the normal path. The previous `9b5a859` package remains the last complete physical/reboot acceptance baseline and is historical evidence only; it does not substitute for current-head physical verification.
