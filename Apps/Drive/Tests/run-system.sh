...
/usr/bin/grep -Fq 'int EDPDirectMFMountFinalizeTeardown(' "${ASYNC_SHIM}"
/usr/bin/grep -Fq 'MFChannelClose(channel)' "${ASYNC_SHIM}"
/usr/bin/grep -Fq 'kDADiskUnmountOptionForce' "${ASYNC_SHIM}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_FINALIZE_DA_FORCE_UNMOUNT_STATUS=' "${ASYNC_SHIM}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_RECEIVE_INTERRUPTED=1 teardown=%d' "${RAW_TRANSPORT}"
/usr/bin/grep -Fq 'EDPDirectMFMountFinalizeTeardown(channel, mountpoint)' "${RAW_TRANSPORT}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_FINALIZE_TEARDOWN_RESULT=' "${RAW_TRANSPORT}"
