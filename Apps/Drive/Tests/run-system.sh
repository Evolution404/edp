...
/usr/bin/grep -Fq 'wait_for_child_exit_bounded "$pid" 50 "adapter-natural-exit-$tag"' "${STORAGE_RUNNER}"
/usr/bin/grep -Fq 'wait_for_child_exit_bounded "$pid" 20 "adapter-term-$tag"' "${STORAGE_RUNNER}"
...
! /usr/bin/grep -Fq '#include <DiskArbitration/DiskArbitration.h>' "${ASYNC_SHIM}"
! /usr/bin/grep -Fq -- '-framework DiskArbitration' "${TRANSPORT_BUILD}"
/usr/bin/grep -Fq 'MFChannelInterrupt(args->channel)' "${ASYNC_SHIM}"
/usr/bin/grep -Fq 'EDPDirectMFMountShutdownRequested' "${ASYNC_SHIM}" "${RAW_TRANSPORT}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_SIGNAL_REFUSED_LIVE_MOUNT=' "${ASYNC_SHIM}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_TERMINATION_AFTER_UNMOUNT=' "${ASYNC_SHIM}"
! /usr/bin/grep -Fq 'EDPDirectMFMountFinalizeTeardown' "${ASYNC_SHIM}" "${RAW_TRANSPORT}"
! /usr/bin/grep -Fq 'kDADiskUnmountOptionForce' "${ASYNC_SHIM}"
! /usr/bin/grep -Fq 'EDPDirectMFMountMarkTransportReleased' "${ASYNC_SHIM}" "${RAW_TRANSPORT}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_DESTROY_RECEIVED=1' "${RAW_TRANSPORT}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_DESTROY_REPLIED=1' "${RAW_TRANSPORT}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_RECEIVE_INTERRUPTED=1 shutdown=%d' "${RAW_TRANSPORT}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_CHANNEL_CLOSE_RESULT=%d errno=%d' "${RAW_TRANSPORT}"
/usr/bin/grep -Fq 'M10 teardown leaked resources at iteration' "${STORAGE_RUNNER}"
...
/usr/bin/grep -Fq 'UI_XCTRACE_RECORD_ATTEMPTS="${EDP_UI_XCTRACE_RECORD_ATTEMPTS:-3}"' "${UI_RUNNER}"
/usr/bin/grep -Fq 'invalid EDP_UI_XCTRACE_RECORD_ATTEMPTS=' "${UI_RUNNER}"
...
/usr/bin/grep -Fq 'only real framework MFMount completion is authoritative readiness' <<<"${MOUNT_WORKER_SECTION}"
...
/usr/bin/grep -Fq 'run_m10' "${STORAGE_RUNNER}"
/usr/bin/grep -Fq 'set +e' "${STORAGE_RUNNER}"
/usr/bin/grep -Fq 'lifecycle_status=$?' "${STORAGE_RUNNER}"
/usr/bin/grep -Fq 'set -e' "${STORAGE_RUNNER}"
