...
/usr/bin/grep -Fq 'MFChannelClose(args->channel)' "${ASYNC_SHIM}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_TEARDOWN_CHANNEL_CLOSE_RESULT=' "${ASYNC_SHIM}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_RECEIVE_INTERRUPTED=1 teardown=%d' "${RAW_TRANSPORT}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_RECEIVE_CHANNEL_CLOSED=1' "${RAW_TRANSPORT}"
/usr/bin/grep -Fq 'DIRECT_MFMOUNT_CHANNEL_CLOSE_OWNED_BY_TEARDOWN=1' "${RAW_TRANSPORT}"
