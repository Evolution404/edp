...
UI_XCTRACE_RECORD_ATTEMPTS="${EDP_UI_XCTRACE_RECORD_ATTEMPTS:-3}"
UI_XCTRACE_EXPORT_TIMEOUT_SECONDS=30
[[ "${UI_XCTRACE_RECORD_ATTEMPTS}" =~ ^[1-9][0-9]*$ ]] || {
  echo "invalid EDP_UI_XCTRACE_RECORD_ATTEMPTS=${UI_XCTRACE_RECORD_ATTEMPTS}" >&2
  exit 64
}
mkdir -p "${BUILD_ROOT}"
cleanup() {
...
grep -Fq 'RESULT=DRIVE_UI_MENU_CREDENTIAL_FOCUS_OK' "${UI_LOG}"
grep -Fq 'SCENARIO=UI_MENU_CREDENTIAL_FOCUS_OK' "${UI_LOG}"
...
grep -Fq 'EDPMenuCredentialEditor(' "${MENU_BAR_SOURCE}"
grep -Fq 'accessibilityIdentifier("menuCredentialPasswordField")' "${MENU_BAR_SOURCE}"
! grep -Fq '.sheet(item: $credentialTarget)' "${MENU_BAR_SOURCE}"
