...
- [ ] `npx tauri dev` 启动：主窗口正常显示；点红色关闭后隐藏到状态栏，点托盘可恢复
- [ ] 密码库页：添加密码（选插入的盘→输密码→保存）→ 密码库列出（脱敏）
- [ ] 设置页：daemon 安装/启动/安全停止/重启/卸载均可用；停止状态跨 Mac 重启保持
- [ ] 普通 U 盘插入后 Finder 状态不变，设备页显示“普通 U 盘，本应用不会操作”
- [ ] 未授权 EDP 盘只显示待配置；逐盘授权后 ≤5s 自动挂载，托盘菜单出现挂载项
- [ ] 全局暂停不卸载现有卷；恢复后立即评估当前已插入的授权设备
