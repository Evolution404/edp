...
| D | 发布可靠性与 recovery 可观测性 | DONE（D1/D2/D4 DONE；D3 仅剩缺失物理负样本 fixture，按发布清单 BLOCKED_BY_FIXTURE 记录） |
