...
- [ ] 当前 claim-continuous runtime-control hardening → commit/push → exact-head CI → self-signed rebuild/install → physical Pause/Resume/Restart（同 PID / `DA_CLAIMED=true` / no fskitd holder / raw reacquire）→ Complete Quit safe-eject/process-exit → final safe-eject/removal/reinsert closure — PENDING。
