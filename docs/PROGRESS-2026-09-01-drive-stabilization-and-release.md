...
- [x] run `33598075171` 在 `f29e710` 上证明 crashed-transport D-state 已消失：storage 不再触发 30 分钟不可中断挂死，而是在约 3 分钟内正常 bounded failure；native/fast/virtual 均 PASS。当前剩余失败前移到普通 M02 remount publication teardown：exact `diskimagesiod` owner snapshot 为 `devices=none`，TERM→KILL 后 backing publication 仍存在。已增加 post-KILL 原 PID/process-state/path 与最终 owner PID/device generation 诊断，下一轮 CI 用于区分不可杀 owner、owner 换代或 stale hdiutil snapshot，residue=0 门槛不变。
