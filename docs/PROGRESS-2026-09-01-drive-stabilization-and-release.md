...
| D | 发布可靠性与 recovery 可观测性 | DONE（D1/D2/D4 DONE；D3 缺失物理负样本 fixture 仍按发布清单 BLOCKED_BY_FIXTURE 记录，不阻断当前 standardEncrypted release-ready 判定） |
...
- [x] 当前 HEAD mandatory reboot gate PASS：exact-head reboot 于 `2026-09-06 21:00:57` 完成；重启前仍物理插着但逻辑 safe-eject 的 Lexar 在 reboot 后重新枚举为 `disk6` 且继续保持 `privilegedAccessReady=false`，service/XPC health、macFUSE FSKit enablement、type2/type4 saved credential 均保持，`rawBusyRecoveryCount=0`、`forcedWholeUnmountCount=0`、`fskitTransientRetryCount=0`。真实拔出后旧 `disk6`/IOKit generation 消失；fresh reinsert 自动恢复同一五因素 stable identity 与 `privilegedAccessReady=true`；post-reboot credential checkpoint、policy round-trip、type1 RO remount、type2/type4 RW persistence 全部 PASS，最终 `RESULT=ALL_THREE_PARTITIONS_CAPABILITY_PERSISTENCE_OK`；final safe eject 后无 EDP mount/transport/raw holder 且 recovery counters 仍全 0。
...
- [x] 第四阶段 release code/package HEAD=`9b5a8595203cf88ff726f9aa08bc62b8c25a8d29` 已 commit/push；exact-head GitHub Actions run `33748594918` 五核心 jobs 5/5 PASS（含 storage M01-M14 与 CI-only UI/system gate）；self-signed Clean.pkg SHA-256=`43659c5fd37cc3cdb5546ab14b71782ea339bd6899209a59570bd119e0e9e264` strict verifier PASS 并已安装。实盘 fresh insertion 为 `disk27`：五因素 identity 完全一致、`DA_CLAIMED=true`、root `lsof` 仅 `edp-drive-service` 持有 `/dev/rdisk27`、无 `fskitd` `/dev/rdisk27s1` holder、`privilegedAccessReady=true`、`rawBusyRecoveryCount=0`。Pause/Resume/Restart 全程 service PID=`34883`、claim 连续，Pause 释放 raw，Resume/Restart 约 2 秒内自动恢复 raw ready，无 EBUSY；foreground App restart 亦保持 claim/raw ready。safe eject PASS，逻辑推出后 App restart/runtime restart 均不 reacquire；真实物理拔出后 IOKit 与 `/dev/disk27` 均消失，再插入自动恢复 early claim/raw ready 且无 fskitd child holder。Complete-Quit end-state 也实证 PASS：safe eject + graceful shutdown + foreground App termination 后 App/service 均不存在，再打开 App 后 service 恢复但仍物理插着的逻辑推出 Lexar 保持 `privilegedAccessReady=false`。mandatory exact-head reboot 已于 `2026-09-04 13:30:00` 完成：service health PASS；重启前逻辑推出状态跨 reboot 保持抑制；真实拔出清除原 generation；再次插入枚举为 `disk4`，五因素 identity 不变，`DA_CLAIMED=true`、`privilegedAccessReady=true`、root `lsof` 仅 `edp-drive-service` 持有 `/dev/rdisk4`、无 `fskitd` child holder、raw errors/busy recovery 为 0；type2/type4 credential 与三分区策略持久；最终 safe eject 后 mount/process/raw-holder residue 均为 0。该阶段曾达到 release-ready，但后续候选仍必须重新通过 exact-head 物理门禁。
- [!] 2026-09-05 对 `7dda539c6d42b2727e96477280f7f9eccdb6cb6f` / Clean.pkg SHA-256 `2af9361d98363d7e797d70e4ad1dc8a0bb9d1423b912cced0698465b62073cdc` 做保留 FDA/FSKit approval 的原地升级后 fresh Lexar insertion，最终 `disk26` 五因素、credential/policy、`privilegedAccessReady=true` 均正确，且终态 root `lsof` 仅 `edp-drive-service` 持有 `/dev/rdisk26`；但 runtime metrics 实际增加 `rawBusyRecoveryCount=1` / `forcedWholeUnmountCount=1`。统一日志证明 `17:14:12.641` FSKit 已对 `/dev/rdisk26s1` 开始检查，`17:14:13.614` whole `disk26` claim 才成功，随后已排队的 mount 仍在 `17:14:13.719` 成功挂载 `/Volumes/启动区`，EDP 于 `17:14:13.745` 被迫 whole-unmount 恢复 raw。故 `7dda539` / `2af936...` 候选作废，D4 重新打开。
- [!] 第一版 mount-gate 修复 exact HEAD=`193ef5a8cd53ed2547779c1bc30b6cdf1cbe56a2`，GitHub Actions run `33957826334` 五核心 jobs 5/5 PASS；certificate-backed Clean.pkg SHA-256=`8a96b8f5fa2cfac5503aa6b7fd0aa1fde3b1bc7ffbd48a40fab88ec5f1df4514` strict verifier PASS 并完成保留 FDA/FSKit approval 的原地升级。2026-09-05 18:15 fresh Lexar insertion 最终五因素、credential、`privilegedAccessReady=true` 正确，且 `rawBusyRecoveryCount=0` / `forcedWholeUnmountCount=0`；但统一日志仍显示 `18:15:22.879` `/dev/disk26s1` 成功 mount，whole `disk26` claim 直到 `18:15:23.610` 才成功，随后 child 于 `18:15:23.939` 卸载。故 `193ef5a` / `8a96b8...` 仍作废：问题已从 EBUSY recovery 收窄为 whole-media peek 本身晚于 child mount approval 的时序竞态。
- [x] 第二版根因/修复：不能依赖 whole-media peek 先于 child mount request。`shouldDenyMount()` 现先检查已有 suppression/gate；若尚未命中，则按 child 所属 `usbRegistryEntryID` 同步定位 exact whole USB，重新读取并验证 LBA0/4/7/11/12 与 IOKit generation。只有完整验证的 `standardEncrypted` 才当场 deny mount；ordinary/unverified/read-failure 均 fail-open。随后正常 whole peek/claim 路径建立独占 claim。新增 S47 `child_mount_approval_can_classify_exact_standard_edp_generation` 与 system ratchet `DRIVE_SYSTEM_MOUNT_APPROVAL_PRE_PEEK_FALLBACK_OK`；本机 S01-S47、10000 sequences/320000 steps、system、fast、`git diff --check` 全绿。
- [x] 最终 release code/package HEAD=`a2fb8745295adf9d84422f91802c868dd95ca16c`，GitHub Actions run `33960406329` 五核心 jobs 5/5 PASS；Clean.pkg SHA-256=`54eba1d6a1e9cb36555dafe58eeccd25740142af61207fa10fa6e2209eed843c` strict verifier PASS。fresh Lexar insertion 与真实拔出/重插均实证 mount approval 对 child FAT 先行 dissent，日志中无 `mounted ... success`，whole-disk claim 成功，`privilegedAccessReady=true`，root raw holder 仅 `edp-drive-service`，`rawBusyRecoveryCount=0` / `forcedWholeUnmountCount=0`。Pause/Resume/Restart 保持同一 service PID，safe eject 后同 generation restart 不重获，真实 removal 后新 generation 自动恢复接管。
- [x] `a2fb874` exact-head reboot 于 `2026-09-05 18:44:02` 完成。纠正验收口径：`com.edp.drive.service` 为 on-demand Mach service，未配置 `RunAtLoad`/`KeepAlive`，因此 EDP Drive 尚未启动时 macOS 正常挂载 FAT16 启动区属于预期行为，不是 release blocker。真正门禁是 EDP Drive/XPC 启动后能卸载系统挂载、恢复 whole-device ownership/raw access 且不进入 recovery；本次已 PASS。post-reboot credential/policy PASS，`functional-all` 的 type1 RO 与 type2/type4 RW persistence PASS，最终 safe eject + residue audit PASS，`RESULT=FIRST_INSTALL_FULL_ACCEPTANCE_BASELINE_OK`。D4 关闭。
...
- [x] 旧 tracker 未勾选项不再作为当前技术债入口；当前总览已按 A/B/C/D DONE 重写；D3 缺失的物理负样本只以 `BLOCKED_BY_FIXTURE` 记录，不冒充 PASS。
