...
- [x] 根因收口：现有 S41 只验证 standardEncrypted classifier/claim selection，没有覆盖异步 `DADiskClaim` 完成前 child mount 已排队的窗口。Apple Disk Arbitration 文档明确 mount approval 才是阻止 automatic mount 的机制；修复增加 exact verified USB-generation early-claim mount gate，在 `DADiskClaim` 前进入保护，claim failure/retire 时释放，claim success 则保持到 exact IOMedia generation 退休。新增 S46 与 system line-order ratchet `DRIVE_SYSTEM_EARLY_CLAIM_MOUNT_GAP_CLOSED`；本机 S01-S46、10000 sequences/320000 steps、system、fast、production EDPCore release build 均 PASS。等待新 exact-head CI 5/5、重新签名出包和 physical replug 验证 `rawBusyRecoveryCount=0` / `forcedWholeUnmountCount=0` 后才能再次关闭 D4。
