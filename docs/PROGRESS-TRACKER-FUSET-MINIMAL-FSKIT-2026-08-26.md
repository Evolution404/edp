...
matrix run `33042781640`（commit `f6921b1`）：精确时间关联已证明 5.3.2/5.3.3 同一调用链：root helper 发起 `unmount(2, MNT_FORCE)` 后约 20-50 ms，`fskitd` 记录 `spontaneous unmount done with error (null)`，随后 Local extension 进入 `-[FSModuleConnector deactivateVolume:numericOptions:replyHandler:]:start`；30 秒后 syscall 仍未返回且 `/dev/disk8` 仍存在。对官方 5.3.3 Local extension 二进制反汇编进一步定位根因：`MFFSModuleLocal.Volume.deactivate()` 首先读取 `Volume.channel`，构造 `MFChannel.Request.destroy`，然后 `await Channel.process(request: .destroy, timeout: .zero)`；该 await 返回后才继续 channel close / virtual-device deactivate。Direct transport 此前对 `FUSE_DESTROY` 只设置 `state.running=false` 并返回 dispatch，未发送 `fuse_out_header` reply，因此 Local extension 永久等待 destroy reply，root `unmount()` 又等待 Local deactivate 完成，形成确定的环形等待。当前修复：`FUSE_DESTROY` 必须先 `send_payload(..., NULL, 0)` 返回 success reply，再结束 receive loop；不改变 privileged VFS teardown 架构。
