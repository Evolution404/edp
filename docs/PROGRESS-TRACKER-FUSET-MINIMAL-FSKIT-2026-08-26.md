...
matrix run `33040455206`（commit `bcfa918`）：两版本在 whole-unmount success 后先调用 `MFChannelClose()`，但随后 eject 仍一致返回 `0xC010 = unix_err(EBUSY)`。日志与 ownership 检查确认 close 后 signal worker 的 retained channel 和 server 主线程的 original channel 尚未 `MFRelease()`；旧 barrier 又把主线程 release 放在 eject 之后，形成确定的 teardown 依赖倒置。下一版增加双向 barrier：worker close 并释放 retained channel → server 离开 receive loop、fsync 并释放 original channel → 通知 `TRANSPORT_RELEASED` → worker 对原 scheduled `DADiskRef` eject → mount-table gate → 通知 teardown complete，server 才退出。
...
matrix run `33042429204`（commit `c5e6460`）：5.3.2/5.3.3 再次都实际取得 runner，并在同一 privileged helper 路径复现：`euid=0`、source `/dev/disk8`、`DIRECT_MFMOUNT_PRIVILEGED_UNMOUNT_CALL=1` 后超过 30 秒仍不返回，快照时 `/dev/disk8` 仍存在；两版本无差异。诊断窗口内还能看到 `fskitd` 的 `spontaneous unmount done with error (null)`、Local extension 的 `deactivateVolume...:start`，并在稍后 server log 中看到 `FUSE_DESTROY`/正常退出，但本轮 `log show --last 3m` 会混入 helper 启动前约 1 秒的事件，不能据此严格证明这些 FSKit 日志由本次 syscall 触发。下一轮仅收紧时间关联：记录 helper 发起前 UTC 秒级起点，并使用 `log show --start <起点>`；同时记录 sudo/helper child PID。架构和 syscall 均不变。`Transport Backend Builder Contract` 同 HEAD run `33042429253` 已绿。

matrix run `33042781640`（commit `f6921b1`）：精确时间关联已证明 5.3.2/5.3.3 同一调用链：root helper 发起 `unmount(2, MNT_FORCE)` 后约 20-50 ms，`fskitd` 记录 `spontaneous unmount done with error (null)`，随后 Local extension 进入 `-[FSModuleConnector deactivateVolume:numericOptions:replyHandler:]:start`；30 秒后 syscall 仍未返回且 `/dev/disk8` 仍存在。对官方 5.3.3 Local extension 二进制反汇编进一步定位根因：`MFFSModuleLocal.Volume.deactivate()` 首先读取 `Volume.channel`，构造 `MFChannel.Request.destroy`，然后 `await Channel.process(request: .destroy, timeout: .zero)`；该 await 返回后才继续 channel close / virtual-device deactivate。Direct transport此前对 `FUSE_DESTROY` 只设置 `state.running=false` 并返回 dispatch，未发送 `fuse_out_header` reply，因此 Local extension 永久等待 destroy reply，root `unmount()` 又等待 Local deactivate 完成，形成确定的环形等待。修复：`FUSE_DESTROY` 必须先 `send_payload(..., NULL, 0)` 返回 success reply，再结束 receive loop；不改变 privileged VFS teardown 架构。

lifecycle fix run `33043742188`（commit `1ad32b1`）：**L7 已完整通过，停止所有 lifecycle 诊断。** 5.3.2 与 5.3.3 两个 matrix job 均实际取得 runner并 success；每个版本都连续完成两轮 `mount → encrypted marker RW → privileged unmount(2, MNT_FORCE) → FUSE_DESTROY success reply → mount table/source teardown → same-mountpoint remount → marker persistence → second privileged unmount`。5.3.2 两轮均记录 `DIRECT_MFMOUNT_PRIVILEGED_UNMOUNT_RESULT=0 errno=0`、`DIRECT_MFMOUNT_DESTROY_REPLIED=1`，最终 `RESULT=DIRECT_SYSTEM_UNMOUNT_REMOUNT_5_3_2_PASS`；5.3.3 同样最终 `RESULT=DIRECT_SYSTEM_UNMOUNT_REMOUNT_5_3_3_PASS`。同 exact HEAD 的 `Direct MFMount Encrypted Local Transport 5.3.2` run `33043742178`、5.3.3 run `33043742169`、`Transport Backend Builder Contract` run `33043742222`、`Direct MFMount Physical Product Adapter Contract` run `33043742179` 均绿。结论：5.3.2/5.3.3 在正确 Direct lifecycle 下都稳定闭环，没有需要继续追踪的版本差异；根因是 Direct server 缺失 destroy reply，不是 macFUSE runtime 版本。按停止条件立即进入 L9/L10 产品化。
