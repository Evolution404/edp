...
WORK_DIR="${RUNNER_TEMP:-/tmp}/edp-crypto-ntfs-readwrite"
...
cleanup() {
  if [[ -n "${DECRYPTED_BSD}" ]]; then
    /usr/sbin/diskutil eject "${DECRYPTED_BSD}" >/dev/null 2>&1 || \
      /usr/sbin/diskutil unmountDisk force "${DECRYPTED_BSD}" >/dev/null 2>&1 || true
  fi
...
attach_decrypted() {
  ...
  /usr/sbin/diskutil mount "${DECRYPTED_BSD}" >/dev/null 2>&1 || true
  for _ in $(seq 1 150); do
    [[ -d "/Volumes/${TEST_VOLUME}" ]] && return 0
    sleep 0.2
  done
  /usr/sbin/diskutil info "${DECRYPTED_BSD}" >&2 || true
  return 1
}
...
log "=== EDP encrypted read/write block -> DiskImages2 -> iBoysoft NTFS ==="
log "macOS=$(/usr/bin/sw_vers -productVersion)"
log "arch=$(/usr/bin/uname -m)"
log "libfuse=$(pkg-config --modversion fuse)"

if ! /usr/sbin/diskutil listFilesystems | /usr/bin/grep -F 'iboysoft_NTFS' >/dev/null; then
  log "FAIL=IBOYSOFT_NTFS_DRIVER_NOT_AVAILABLE"
  exit 69
fi
...
/opt/homebrew/sbin/mkntfs -F -L "${TEST_VOLUME}" "${PLAIN_IMAGE}" \
  >>"${REPORT_FILE}" 2>&1
log "RESULT=SYNTHETIC_NTFS_FIXTURE_PREPARED"
...
RW_INFO="$(/usr/sbin/diskutil info "${DECRYPTED_BSD}")"
printf '%s\n' "${RW_INFO}" | /usr/bin/grep -Eq 'File System Personality:[[:space:]]+iboysoft_NTFS'
printf '%s\n' "${RW_INFO}" | /usr/bin/grep -Eq 'Volume Read-Only:[[:space:]]+No'
log "RESULT=IBOYSOFT_NTFS_MOUNTED_READWRITE"

PROOF_PATH="/Volumes/${TEST_VOLUME}/edp-readwrite-proof.bin"
...
log "RESULT=IBOYSOFT_NTFS_FILE_WRITE_READBACK_OK"
...
log "RESULT=EDP_NTFS_WRITE_SURVIVES_FULL_REMOUNT"

eject_decrypted
stop_fuse
log "RESULT=EDP_CRYPTO_DISKIMAGES2_IBOYSOFT_NTFS_READWRITE_E2E_OK"
